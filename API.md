...
* `2017` - комната не найдена или игра не запущена
* `2018` - пользователь не является владельцем комнаты
...
* `706` - персонаж не найден
* `2008` - пользователь отсутствует в комнате
* `2017` - комната не найдена или игра не начата
* `2018` - пользователь не является владельцем комнаты
* `5001` - тип бота не найден
* `5002` - ошибка создания бота
* `5003` - неверный формат данных бота
...
```
{
    token: string, - токен пользователя в комнате
}
```
**Успешный ответ**
```
...
* `705` - пользователь не найден
* `2008` - пользователь отсутствует в комнате
* `5006` - боты не найдены в комнате
...
```
{
    ...
    botData: string - JSON с новыми данными бота {"hp":,"x":,"y":}
}
```
**Успешный ответ**
```
...
**Особенности:**
```
  Если HP ≤ 0, бот автоматически удаляется
```
